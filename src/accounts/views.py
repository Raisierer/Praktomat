    ...
    if (not settings.REGISTRATION_POSSIBLE) or get_settings().deny_registration_from < datetime.now():
        ...
        if request.method == 'POST':
            form = MyRegistrationForm(
                request.POST, domain=RequestSite(request).domain, use_https=True)
            if form.is_valid():
                form.save()
...
def activation_allow(request, user_id):
    ...
    send_mail(_("Account activation on %s") %
              settings.SITE_NAME, t.render(c), None, [user.email])
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, "The import was successfull. %i users imported." % imported_user.count())
                if form.cleaned_data['require_reactivation']:
                    for user in [user for user in imported_user if user.is_active]:
                        ...
                        if form.cleaned_data['send_reactivation_email']:
                            ...
                            send_mail(_("Account activation on %s") % settings.SITE_NAME, t.render(
                                c), None, [user.email])
...
def import_tutorial_assignment(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            file.seek(0)
            try:
                reader = csv.reader(io.StringIO(file.read().decode('utf-8')), delimiter=str(
                    form.cleaned_data['delimiter']), quotechar=str(form.cleaned_data['quotechar']))
            ...
            for row in reader:
                try:
                    matching_users = User.objects.filter(
                        mat_number=row[form.cleaned_data['mat_coloum']])
                    if not matching_users.exists():
                        not_present += 1
                        continue
                    user = matching_users.get()
                    tutorial = Tutorial.objects.get(
                        name=row[form.cleaned_data['name_coloum']])
                    ...
                    failed += 1
            #assert False
            messages.warning(request, "%i assignments were imported successfully, %i users not found, %i failed." % (
                succeeded, not_present, failed))
            return HttpResponseRedirect(reverse('admin:accounts_user_changelist'))
    else:
        ...
        if form.is_valid():
            ...
            for row in reader:
                try:
                    ...
                    failed += 1
            #assert False
            messages.warning(request, "%i user texts were imported successfully, %i users not found, %i failed." % (
                succeeded, not_present, failed))
            return HttpResponseRedirect(reverse('admin:accounts_user_changelist'))
    else:
